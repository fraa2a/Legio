...
04A validates the exact versioned source document, applies a size limit, and retrieves only the fixed HTTPS URL `https://source.taxphobia.top/store.json`. The endpoint may return an availability error until the project publishes `store.json`.
